...
__global__ void msfunction(float *x, float *y) {
    float temp[DIMS], m;
    int i = blockIdx.x * BLK + threadIdx.x * blockDim.x  + threadIdx.y;
    if (i < N) {
        memcpy(y+DIMS*i, x+DIMS*i, DIMS*sizeof(float));
        do {
            meanshift(x, y+DIMS*i, temp);
            m = sqrt(distance(temp, y+DIMS*i));
            memcpy(y+DIMS*i, temp, DIMS*sizeof(float));
        } while (m>epsilon);
    }
}
